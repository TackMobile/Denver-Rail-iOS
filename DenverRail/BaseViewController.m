...
@synthesize topLevelSliderTopConstraint, topLevelSliderBottomConstraint, pickerViewTopConstraint, topSectionTopConstraint;
...
- (void)viewDidLoad {
  ...
  self.searchViewController.view.frame = CGRectMake(0, CGRectGetMaxY(self.stationNameView.frame),
                                                    CGRectGetWidth(self.contentSubView.frame),
                                                    CGRectGetHeight(self.contentSubView.frame) - CGRectGetMaxY(self.stationNameView.frame));
...
-(void)toggleManualMode {
  ...
  if (isAutoMode) {
    ...
    topSectionTopConstraint.constant = 0;
    ...
    topSectionTopConstraint.constant = -46;
    [UIView animateWithDuration:0.2
                     animations:^{
                       self.scheduleViewController.view.frame = CGRectMake(24,
                                                                           self.scheduleViewController.view.frame.origin.y - screenAdjust,
...
-(void)showPickerView {
  ...
  if (self.pickerView.frame.origin.y >= [[UIScreen mainScreen] applicationFrame].size.height) {
    ...
    pickerViewTopConstraint.constant = -self.pickerView.frame.size.height;
    [UIView animateWithDuration:0.2
                     animations:^{
                       [self.view layoutIfNeeded];
                     }];
...
// Show top level slider view
-(void)showTopLevelSlider {
  topLevelSliderTopConstraint.constant = 0;
  topLevelSliderBottomConstraint.constant = 0;
  [UIView animateWithDuration:0.2
                   animations:^{
                     [self.view layoutIfNeeded];
                   }];
...
// Hide top level slider view
-(void)hideTopLevelSlider {
  topLevelSliderTopConstraint.constant = [[UIScreen mainScreen] applicationFrame].size.height + self.topSection.frame.size.height;
  topLevelSliderBottomConstraint.constant = -[[UIScreen mainScreen] applicationFrame].size.height + self.topSection.frame.size.height;
  [UIView animateWithDuration:0.2
                   animations:^{
                     [self.view layoutIfNeeded];
                   }];
